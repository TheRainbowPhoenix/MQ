...
void GUI::Render(mqController *controller)
{
    ...
    bool initialized = omach->initialized;
    ...
    if(ImGui::BeginCustomMenuBar()) {
        ...
        if(ImGui::BeginCustomMenuChild("##menutools", {0,0}, {1,4})) {
            ...
            if(ImGui::IconButton(5, "Screenshot", !initialized))
                gui.actions.recordScreenshot = true;
            ImGui::SameLine(0, 6);

#if MQ_VIDEO_FFMPEG
            int status = gui.recorder.status();
            ...
            if(initialized && notstarted) {
                ...
                if(ImGui::IconButton(7, "Resume recording"))
                    gui.actions.recordUnpause = true;
            }
            else ImGui::IconButton(7, "Start recording", true);
            ImGui::SameLine(0, 6);

            if(ImGui::IconButton(8, "Pause recording", !recording))
                gui.actions.recordPause = true;
            ImGui::SameLine(0, 6);

            if(ImGui::IconButton(9, "Stop recording", notstarted))
                gui.actions.recordStop = true;
            ImGui::SameLine(0, 6);

            if(recording || paused) {
                char buffer[128];
                mqRecordStats const *stats = gui.recorder.stats();
                snprintf(buffer, sizeof buffer, "%02d:%02d:%02d",
                    stats->time_min, stats->time_sec, stats->time_ms);
...
void GUI::DockWindowsStyle1(GUIWindowSet const &Windows, ImGuiID dock)
{
    ...
    auto dock_left_top_right = ImGui::DockBuilderSplitNode(dock_left_top,
        ImGuiDir_Right, 0.60f, nullptr, &dock_left_top);
...
void GUI::ResetState()
{
    ...
    if(recorder.status() == MQ_RECORD_STATUS_RECORDING ||
...
void RecordWindow::renderContents(mqMachine *omach)
{
    ...
    ImGui::Checkbox2(
        "Record frames only when the program is running",
        &gui.recordOnlyWhenRunning);
    ...
    if(!uninit && notstarted) {
        if(ImGui::ButtonWSized("Start", w_button))
            gui.actions.recordStart = true;
    }
    else if(!uninit && paused) {
        if(ImGui::ButtonWSized("Resume", w_button))
            gui.actions.recordUnpause = true;
    }
    else ImGui::ButtonWSized("Start", w_button, true);
    ImGui::SameLine(0, style.ItemInnerSpacing.x);

    if(ImGui::ButtonWSized("Pause", w_button, uninit || !recording))
        gui.actions.recordPause = true;
    ImGui::SameLine(0, style.ItemInnerSpacing.x);

    if(ImGui::ButtonWSized("Stop", w_button, uninit || notstarted))
        gui.actions.recordStop = true;
    ImGui::Spacing();
    ...
    if(recording || paused) {
        ...
        char const *message = "Recording...";
        if(paused)
            message = "Paused";
        else if(gui.recordOnlyWhenRunning && omach->cyclesPending == 0)
            message = "Waiting for the add-in...";
        snprintf(
            buffer, 128,
            "%s (%02d:%02d:%02d)", message,
            stats->time_min,
            stats->time_sec,
            stats->time_ms
        );
