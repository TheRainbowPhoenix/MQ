...
bool mq_casiowin_setup(mqMachine *mach, enum mqCasiowin_Version version)
{
    ...
    mqChunk *ch = mq_memory_getOrCreateChunk(mach->memory, OSBase & ~0xfffff);
    if(!ch)
        return false;
    mqMMIOPage *mmpg_os = mq_chunk_getOrCreateMMIOPage(ch, OSBase, 0, 1);
    if(!mmpg_os)
        return false;
    mqMMIOPage *mmpg_eboot =
        mq_chunk_getOrCreateMMIOPage(ch, OSBase - 0x1000, 0, 1);
    if(!mmpg_eboot)
        return false;
